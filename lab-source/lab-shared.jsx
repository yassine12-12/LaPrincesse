// Shared atoms across the 3 LaPrincesse Lab directions.
// Static "feels-3D" nail renders using radial gradients + filters.

// ─────────────────────────────────────────────────────────────
// NailRender — a single nail tip drawn with CSS, given a "material"
// material: chrome | onyx | pearl | crystal | blood | bone | glass
// shape:    almond | coffin | square | stiletto | oval
// length:   short | medium | long | xlong
// ─────────────────────────────────────────────────────────────
function nailPath(shape, w, h) {
  // SVG path of one nail tip viewed from above. Origin top-left, w x h.
  const r = w / 2;
  switch (shape) {
    case 'square':
...
const LENGTH_MAP = { short: 0.9, medium: 1.25, long: 1.65, xlong: 2.0 };

function NailRender({
  shape = 'almond', length = 'long', material = 'chrome',
  width = 64, accent = null, gem = false, chromeStripe = false,
  style = {},
}) {
    ...
    <div style={{ position: 'relative', width, height: baseH, ...style }}>
      <svg width={width} height={baseH} style={{ position: 'absolute', inset: 0 }}>
        ...
        {chromeStripe && (
          <foreignObject width={width} height={baseH} clipPath={`url(#clip-${id})`}>
            <div style={{
              position: 'absolute', left: '40%', top: 0, width: '20%', height: '100%',
...
// ─────────────────────────────────────────────────────────────
// HandLayout — 5 nails rising from the bottom of a frame, scaled
// per finger. A "stage" for hero renders.
// ─────────────────────────────────────────────────────────────
function HandLayout({ shape, material, length = 'long', accent, gem = [false,false,false,false,false], chromeStripe = [false,false,false,false,false], scale = 1, style = {} }) {
  // proportions for thumb, index, middle, ring, pinky
  const fingers = [
  ...
  return (
    <div style={{ position: 'relative', width: 256 * scale, height: 220 * scale, ...style }}>
      {fingers.map((f, i) => (
        <div key={i} style={{
          position: 'absolute', left: f.x * scale, bottom: -f.y * scale,
        }}>
          <NailRender
            shape={shape} material={material} length={length}
            accent={accent} gem={gem[i]} chromeStripe={chromeStripe[i]}
            width={f.w}
          />
        </div>
      ))}
...
Object.assign(window, { NailRender, HandLayout, MATERIALS, nailPath });
