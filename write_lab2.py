lab_content = r"""import { createFileRoute, Link } from '@tanstack/react-router';
...
// ── Color palette ─────────────────────────────────────────────
type ColorEntry = { k: string; n: string; sub: string; bg: string };
const COLOR_CATEGORIES: { label: string; colors: ColorEntry[] }[] = [
  {
    label: 'Metallics',
    colors: [
      { k: 'chrome',   n: 'Chrome',    sub: 'Mirror',     bg: 'linear-gradient(180deg,#fafafa 0%,#9aa0a6 28%,#1a1a1c 55%,#6e7177 78%,#e8e9ec 100%)' },
      { k: 'platinum', n: 'Platinum',  sub: 'Icy',        bg: 'linear-gradient(180deg,#f8f8ff 0%,#d0d0e8 40%,#a0a0c0 100%)' },
      { k: 'silver',   n: 'Silver',    sub: 'Cool',       bg: 'linear-gradient(180deg,#f0f0f8 0%,#b0b0c8 40%,#707080 100%)' },
      { k: 'gold',     n: 'Gold',      sub: 'Yellow',     bg: 'linear-gradient(180deg,#fff8c0 0%,#d4a840 40%,#8a6010 100%)' },
      { k: 'rosegold', n: 'Rose Gold', sub: 'Warm',       bg: 'linear-gradient(180deg,#ffd8d0 0%,#d4886a 40%,#8a4030 100%)' },
      { k: 'bronze',   n: 'Bronze',    sub: 'Antique',    bg: 'linear-gradient(180deg,#e8c890 0%,#a07840 40%,#604818 100%)' },
      { k: 'copper',   n: 'Copper',    sub: 'Warm metal', bg: 'linear-gradient(180deg,#f0c080 0%,#b87040 40%,#703018 100%)' },
      { k: 'holo',     n: 'Holo',      sub: 'Rainbow',    bg: 'linear-gradient(135deg,#f0d0f8 0%,#d0e8ff 25%,#d8ffd0 50%,#fff8d0 75%,#ffd0e8 100%)' },
    ],
  },
  {
    label: 'Classics',
    colors: [
      { k: 'nude',     n: 'Nude',      sub: 'Skin tone',  bg: 'linear-gradient(180deg,#e8b898 0%,#c8906a 50%,#a06040 100%)' },
      { k: 'french',   n: 'French',    sub: 'Sheer pink', bg: 'linear-gradient(180deg,#fff8f4 0%,#f4e4dc 50%,#e8d4c8 100%)' },
      { k: 'red',      n: 'Red',       sub: 'Classic',    bg: 'linear-gradient(180deg,#e02020 0%,#a00a10 50%,#600008 100%)' },
      { k: 'cherry',   n: 'Cherry',    sub: 'Deep red',   bg: 'linear-gradient(180deg,#c02030 0%,#6a0010 50%,#380008 100%)' },
      { k: 'bordeaux', n: 'Bordeaux',  sub: 'Dark rouge', bg: 'linear-gradient(180deg,#5a1015 0%,#3a0608 50%,#180204 100%)' },
      { k: 'navy',     n: 'Navy',      sub: 'Deep blue',  bg: 'linear-gradient(180deg,#182858 0%,#08102a 50%,#020810 100%)' },
      { k: 'cobalt',   n: 'Cobalt',    sub: 'Royal blue', bg: 'linear-gradient(180deg,#2040d0 0%,#0a28a0 50%,#041060 100%)' },
      { k: 'forest',   n: 'Forest',    sub: 'Dark green', bg: 'linear-gradient(180deg,#1a5028 0%,#0a2a10 50%,#041008 100%)' },
      { k: 'onyx',     n: 'Onyx',      sub: 'Liquid blk', bg: 'linear-gradient(180deg,#2b2b2e 0%,#0a0a0b 50%,#1a1a1d 100%)' },
      { k: 'matte',    n: 'Matte Blk', sub: 'Ultra-flat', bg: 'linear-gradient(180deg,#141414 0%,#080808 100%)' },
    ],
  },
  {
    label: 'Pastels',
    colors: [
      { k: 'babypink', n: 'Baby Pink', sub: 'Soft',       bg: 'linear-gradient(180deg,#fce0e8 0%,#f8c8d4 50%,#e8a8b8 100%)' },
      { k: 'blush',    n: 'Blush',     sub: 'Warm rose',  bg: 'linear-gradient(180deg,#fcd8d8 0%,#f4c0c4 50%,#dca0a4 100%)' },
      { k: 'lavender', n: 'Lavender',  sub: 'Lilac',      bg: 'linear-gradient(180deg,#ece0f8 0%,#c8b0e4 50%,#a890c8 100%)' },
      { k: 'lilac',    n: 'Lilac',     sub: 'Soft violet', bg: 'linear-gradient(180deg,#f0e0ff 0%,#d4b0f4 50%,#b490d8 100%)' },
      { k: 'mint',     n: 'Mint',      sub: 'Fresh',      bg: 'linear-gradient(180deg,#c8f8e8 0%,#9ae0c4 50%,#70c0a0 100%)' },
      { k: 'peach',    n: 'Peach',     sub: 'Warm',       bg: 'linear-gradient(180deg,#fce8d4 0%,#f4c0a0 50%,#d8987c 100%)' },
      { k: 'sky',      n: 'Sky Blue',  sub: 'Powder',     bg: 'linear-gradient(180deg,#dceeff 0%,#a8c8f4 50%,#88a8d8 100%)' },
      { k: 'butter',   n: 'Butter',    sub: 'Soft yellow', bg: 'linear-gradient(180deg,#fefce0 0%,#f4e4a0 50%,#d8c870 100%)' },
    ],
  },
  {
    label: 'Neons',
    colors: [
      { k: 'neonpink',   n: 'El. Pink',   sub: 'Electric', bg: 'linear-gradient(180deg,#ff80c0 0%,#ff1080 50%,#c00060 100%)' },
      { k: 'neonlime',   n: 'Acid Lime',  sub: 'Acid',     bg: 'linear-gradient(180deg,#b0ff40 0%,#70e800 50%,#40a000 100%)' },
      { k: 'neoncyan',   n: 'Laser Cyan', sub: 'Laser',    bg: 'linear-gradient(180deg,#80ffff 0%,#00e8e8 50%,#00a0a0 100%)' },
      { k: 'neonorange', n: 'Fire',       sub: 'Blaze',    bg: 'linear-gradient(180deg,#ffa060 0%,#ff5800 50%,#c03000 100%)' },
      { k: 'neonpurple', n: 'UV Purple',  sub: 'UV',       bg: 'linear-gradient(180deg,#c080ff 0%,#8800ff 50%,#5000b0 100%)' },
    ],
  },
  {
    label: 'Special',
    colors: [
      { k: 'pearl',    n: 'Pearl',     sub: 'Iridescent', bg: 'linear-gradient(180deg,#fff 0%,#f3ecdf 45%,#e0d4c3 100%)' },
      { k: 'crystal',  n: 'Crystal',   sub: 'Optical',    bg: 'linear-gradient(180deg,rgba(220,234,248,0.75) 0%,rgba(155,180,210,0.55) 50%,rgba(220,234,248,0.75) 100%)' },
      { k: 'glass',    n: 'Glass',     sub: 'Transparent', bg: 'linear-gradient(180deg,rgba(255,255,255,0.18) 0%,rgba(255,255,255,0.06) 100%)' },
      { k: 'glitter',  n: 'Glitter',   sub: 'Sparkle',    bg: 'linear-gradient(135deg,#d0d0ff 0%,#e0c0ff 33%,#d0e0ff 66%,#ffe0d0 100%)' },
      { k: 'bone',     n: 'Bone',      sub: 'Matte nude', bg: 'linear-gradient(180deg,#f5efe2 0%,#e6dbc6 100%)' },
      { k: 'jet',      n: 'Jet',       sub: 'Ultra-matte', bg: 'linear-gradient(180deg,#3a3a3e 0%,#000 50%,#1a1a1c 100%)' },
    ],
  },
];
...
const GEM_LIBRARY: { id: string; name: string; tint: string }[] = [
  ...
  { id: 'diamond',  name: 'Diamond',  tint: 'radial-gradient(circle at 35% 28%, #f8fcff 0%, #c0d0e8 40%, #8090b0 80%)' },
  { id: 'pearl',    name: 'Pearl',    tint: 'radial-gradient(circle at 30% 24%, #fff 0%, #f3ecdf 50%, #c8b8a0 100%)' },
  { id: 'ruby',     name: 'Ruby',     tint: 'radial-gradient(circle at 35% 28%, #ff8888 0%, #8a0a12 60%, #aa2030 100%)' },
  { id: 'sapphire', name: 'Sapphire', tint: 'radial-gradient(circle at 35% 28%, #8090ff 0%, #08163a 60%, #2040a0 100%)' },
  { id: 'emerald',  name: 'Emerald',  tint: 'radial-gradient(circle at 35% 28%, #88ffb0 0%, #08402a 60%, #1a7050 100%)' },
  { id: 'amethyst', name: 'Amethyst', tint: 'radial-gradient(circle at 35% 28%, #e0a0ff 0%, #500858 60%, #9030a0 100%)' },
  { id: 'topaz',    name: 'Topaz',    tint: 'radial-gradient(circle at 35% 28%, #ffe080 0%, #a07828 60%, #806020 100%)' },
  ...
  { id: 'silver',   name: 'Silver',   tint: 'radial-gradient(circle at 35% 28%, #fff 0%, #d8d8e0 40%, #888 100%)' },
];
...
export default function LabPage() {
  ...
  const [material, setMaterial] = useState<string>('chrome');
  ...
  const [gems, setGems] = useState<GemsMap>({
    ring: [{ id: 'crystal' as GemId, shape: 'round' as GemShapeId, zone: 'top', x: 0, y: 0.5 }],
  });
  ...
  function clearFinger() { setGems((prev) => ({ ...prev, [activeFinger]: [] })); }
  function clearAll() { setGems({}); }
        ...
        <div style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', top: 20, left: 20, right: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10 }}>
            <div style={{ display: 'flex', gap: 6 }}>
              {['Perspective', 'Top', 'Side'].map((v, i) => (
                <div key={v} style={{ fontSize: 9, letterSpacing: 1.6, textTransform: 'uppercase', padding: '5px 10px', borderRadius: 100, background: i === 0 ? 'rgba(255,255,255,0.08)' : 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: i === 0 ? '#fff' : 'rgba(255,255,255,0.4)', backdropFilter: 'blur(20px)', fontFamily: sans }}>{v}</div>
              ))}
            </div>
          ...
          {/* Soft glow */}
          <div style={{ position: 'absolute', top: '15%', left: '50%', transform: 'translateX(-50%)', width: 480, height: 480, borderRadius: '50%', background: 'radial-gradient(circle, rgba(180,200,230,0.12), transparent 65%)', pointerEvents: 'none' }} />
          ...
          {/* Bottom caption */}
          <div style={{ position: 'absolute', bottom: 24, left: 22, right: 22, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', zIndex: 10, pointerEvents: 'none' }}>
            <div>
              ...
              <div style={{ fontFamily: fraunces, fontStyle: 'italic', fontSize: 28, fontWeight: 300, marginTop: 4, letterSpacing: -0.5 }}>{matName}, {length}.</div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', borderLeft: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '14px 20px 0', flexShrink: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <div style={{ fontFamily: fraunces, fontStyle: 'italic', fontSize: 11, color: 'rgba(255,255,255,0.6)' }}>on <span style={{ color: '#fff' }}>{activeFinger}</span></div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, rowGap: 12, paddingBottom: 12 }}>
              {GEM_LIBRARY.map((g) => (
                <button key={g.id} onClick={() => addGem(g.id)} title={`Add ${g.name} gem`} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', textAlign: 'center' }}>
                  ...
                  <div style={{ marginTop: 4, fontFamily: fraunces, fontStyle: 'italic', fontSize: 9, color: 'rgba(255,255,255,0.55)' }}>{g.name}</div>
                </button>
              ))}
            </div>

            <div style={{ marginBottom: 6, fontFamily: fraunces, fontStyle: 'italic', fontSize: 10, color: 'rgba(255,255,255,0.35)', lineHeight: 1.4 }}>
              {(gems[activeFinger] ?? []).length} gem{(gems[activeFinger] ?? []).length !== 1 ? 's' : ''} on {activeFinger} - {totalGems} total
            </div>
          </div>
          ...
          <div style={{ padding: '14px 20px', borderTop: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            <div style={{ padding: '16px 16px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12 }}>
                <div>
                  ...
                  <div style={{ fontFamily: fraunces, fontStyle: 'italic', fontSize: 20, marginTop: 1 }}>{totalGems > 3 ? '2h 40m' : totalGems > 0 ? '2h 10m' : '1h 20m'}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.45)', fontFamily: sans }}>Tier</div>
                  <div style={{ fontFamily: fraunces, fontStyle: 'italic', fontSize: 20, marginTop: 1 }}>{totalGems > 3 ? 'III' : totalGems > 0 ? 'II' : 'I'}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.45)', fontFamily: sans }}>Price</div>
                  <div style={{ fontFamily: fraunces, fontStyle: 'italic', fontSize: 20, marginTop: 1 }}>{totalGems > 3 ? '165 EUR' : totalGems > 0 ? '140 EUR' : '80 EUR'}</div>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button style={{ flex: 1, padding: '13px 0', borderRadius: 100, background: '#fff', color: '#000', border: 'none', fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', fontFamily: sans }}>Reserve</button>
              <button style={{ padding: '13px 16px', borderRadius: 100, background: 'transparent', color: '#fff', border: '1px solid rgba(255,255,255,0.18)', fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', fontWeight: 500, cursor: 'pointer', fontFamily: sans }}>Save</button>
            </div>
          </div>
...
with open('/Users/yassinekraiem/Documents/LaPrincesse/frontend/src/routes/lab.tsx', 'w') as f:
    f.write(lab_content)
print("lab.tsx written:", len(lab_content), "bytes")
